const state = { rows: [], history: {}, sort: "score", descending: true, sector: "all", search: "", historySymbols: new Set(["SPY"]) };
...
function renderHistory() {
  ...
  const x = (index) => left + (dates.length === 1 ? innerWidth / 2 : index * innerWidth / (dates.length - 1));
  ...
  const paths = series.map((row, seriesIndex) => { const color = colors[seriesIndex % colors.length]; const line = row.observations.map((point, index) => { const dateIndex = dates.indexOf(point.date); return `${index ? "L" : "M"}${x(dateIndex).toFixed(1)},${y(point.score).toFixed(1)}`; }).join(" "); return `<path class="score-line" stroke="${color}" d="${line}"><title>${row.symbol}</title></path>`; }).join("");
  const dots = series.length <= 6 ? series.map((row, seriesIndex) => row.observations.map((point) => `<circle class="score-point" fill="${colors[seriesIndex % colors.length]}" stroke="${colors[seriesIndex % colors.length]}" cx="${x(dates.indexOf(point.date))}" cy="${y(point.score)}" r="4"><title>${row.symbol} · ${point.date}: ${scoreText(point.score)}</title></circle>`).join("")).join("") : "";
