...
function render() {
  ...
  byId("radar-rows").innerHTML = shown.map((r) => `<tr><td>${r.symbol}</td><td>${r.sector}</td><td><span class="radar-signal ${r.radarScore >= 75 ? "accelerating" : r.radarScore >= 55 ? "leading" : r.radarScore < 25 ? "weak" : ""}">${r.outlook}</span></td><td class="radar-reason">${r.reason}</td><td><details class="radar-details"><summary>Vezi analiza</summary><div><strong>Confirmări</strong><ul>${r.confirmations.length ? r.confirmations.map((item) => `<li>${item}</li>`).join("") : "<li>Nicio confirmare puternică.</li>"}</ul><strong>Riscuri</strong><ul>${r.risks.length ? r.risks.map((item) => `<li>${item}</li>`).join("") : "<li>Niciun risc tehnic major din criteriile urmărite.</li>"}</ul></div></details></td><td class="confidence ${r.confidence === "Ridicată" ? "pos" : r.confidence === "Redusă" ? "neg" : ""}">${r.confidence}</td><td class="${tone(r.scoreDelta5d)}">${scoreText(r.scoreDelta5d)}</td><td class="radar-score ${r.radarScore >= 55 ? "pos" : r.radarScore < 25 ? "neg" : ""}">${r.radarScore}</td><td class="${tone(r.relative1m)}">${pct.format(r.relative1m / 100)}</td><td class="${tone(r.relative3m)}">${pct.format(r.relative3m / 100)}</td><td>${r.rvol == null ? "—" : r.rvol.toFixed(1)}</td></tr>`).join("");
