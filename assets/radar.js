...
function radar(row, history) {
  ...
  const confirmations = [trend === 25, relative === 25, momentum === 20, acceleration === 20, volume === 10].filter(Boolean).length;
  const confidence = confirmations >= 4 ? "Ridicată" : confirmations >= 3 ? "Medie" : "Redusă";
  ...
  return { ...row, radarScore, outlook, confidence, scoreDelta5d, reason };
}
function render() {
