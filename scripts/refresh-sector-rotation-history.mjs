...
const previousRotationScore = (history, symbol, sessions) => { const dates = Object.keys(history).sort(); return dates.length > sessions ? history[dates.at(-(sessions + 1))]?.[symbol]?.score : null; };
...
for (const row of latest.rows.filter((item) => SECTORS.has(item.symbol))) {
  ...
  const baseScore = trend + rs + momentum + acceleration + volume + macroFactor.value;
  const oneWeek = previousRotationScore(rotationHistory, row.symbol, 5), fourWeeks = previousRotationScore(rotationHistory, row.symbol, 20);
  const scoreMomentum = (baseScore > oneWeek ? 3 : 0) + (baseScore > fourWeeks ? 2 : 0) - (baseScore < oneWeek ? 3 : 0) - (baseScore < fourWeeks ? 2 : 0);
  const score = Math.max(0, Math.min(100, baseScore + scoreMomentum));
